...
import { NutritionistStats } from "@/redux/features/api/NutritionistDashboard/nutritionistOverviewApi";
...
export default function StatCards({ stats, isLoading }: StatCardsProps) {
  ...
  const handleCardClick = (key: string) => {
    if (key === "needing_attention") {
      router.push("/nutritionist-dashboard/clients"); // Mapping to general clients for now
    }
    if (key === "active_clients") {
      router.push("/nutritionist-dashboard/clients");
    }
    if (key === "pending_messages") {
      router.push("/nutritionist-dashboard/messages");
    }
    if (key === "todays_checkins") {
      router.push("/nutritionist-dashboard/overview"); // Placeholder
    }
  };
      ...
      {statCardsData.map((stat, index) => (
        ...
        >
          <div className="flex flex-col items-start gap-4">
            <div className="p-3 w-13 h-13 bg-[#9AAEB233] rounded-lg text-[#0FA4A9]">
              {stat.icon}
            </div>
