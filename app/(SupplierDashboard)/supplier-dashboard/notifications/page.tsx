...
const getNotificationIcon = (type: string | null) => {
  ...
  if (t.includes("motivation") || t.includes("message")) return <MessageCircle size={24} className="text-[#3A86FF]" />;
  ...
  return <Bell size={24} className="text-[#3A86FF]" />; // Default icon to Supplier Blue
};
    ...
    <div className="flex flex-col min-h-[calc(100vh-80px)] p-6 md:p-10 max-w-6xl mx-auto w-full pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        <div className="space-y-2">
          <h1 className="text-2xl md:text-3xl font-bold text-[#041228]">Notifications</h1>
          <p className="text-[#5F6F73] font-medium leading-relaxed">
            Manage your orders, product approvals, and communications.
          </p>
        </div>
        <div className="flex items-center gap-4">
          <button
            onClick={handleMarkAllAsRead}
            className="bg-[#3A86FF] text-white px-6 py-2.5 rounded-xl font-bold hover:bg-opacity-90 transition-all text-sm cursor-pointer shadow-sm shadow-[#3A86FF]/20"
          >
            Mark all as read
          </button>
          <button
      ...
      <div className="flex flex-col gap-5">
            ...
            >
              ...
              <div className="flex flex-col gap-1.5 flex-1 pr-12 sm:pr-0">
                <div className="flex items-start justify-between gap-2">
                  <h3 className="text-lg font-bold text-[#041228] leading-tight flex items-center gap-2">
                    {notif.title || "Notification"}
                    {!notif.read_at && (
                      <span className="w-2.5 h-2.5 rounded-full bg-[#3A86FF] inline-block shadow-[0_0_10px_rgba(58,134,255,0.4)]"></span>
                    )}
                  </h3>
              ...
              <div className="absolute right-6 top-1/2 -translate-y-1/2 sm:static sm:translate-y-0 flex items-center gap-3 ml-4">
                ...
                <div className="w-12 h-12 rounded-xl border border-gray-100 flex items-center justify-center text-gray-400 group-hover:border-[#3A86FF] group-hover:text-[#3A86FF] transition-all hidden sm:flex">
        ...
        {notifications.length === 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="bg-white rounded-3xl p-20 border border-[#E4EFFF] flex flex-col items-center justify-center gap-6 text-center mt-4 shadow-sm"
          >
            <div className="w-24 h-24 rounded-full bg-gray-50 flex items-center justify-center">
              <Bell size={48} className="text-gray-200" />
            </div>
            <div>
              <h3 className="text-xl font-bold text-[#041228] mb-2">No notifications yet</h3>
              <p className="text-[#5F6F73] font-medium">We'll let you know when something important happens.</p>
            </div>
          </motion.div>
        )}
      </div>
